...
- **frontend** (`frontend/`) is a **React + Vite** single-page app (JavaScript). It is built to static files and served by nginx inside the frontend container (`frontend/Dockerfile` is a multi-stage build → nginx; `frontend/nginx.conf` adds the SPA `try_files … /index.html` fallback). Routing is client-side via `react-router-dom` (`src/App.jsx`); pages live in `src/pages/`. All data access goes through `src/api/client.js` (base URL `/api`, `fetch` with `credentials: 'include'`). For local dev outside Docker, `vite dev` proxies `/api` (see `vite.config.js`).
  - **Styling** is **Tailwind CSS v4** (no Bootstrap) via the `@tailwindcss/vite` plugin (`vite.config.js`). The only stylesheet is `src/styles/global.css`: it `@import 'tailwindcss'`, defines the ocean-blue palette + system font as design tokens in an `@theme` block (`--color-ocean-*`), and declares a small set of `@layer components` classes used across the app — `.btn`/`.btn-primary`/`.btn-secondary`/`.btn-danger`, `.input`, `.field-label`. Everything else is inline utility classes. Conventions: use `ocean-*` for brand color, **slate** for neutral (untinted) borders — always set an explicit border color since Tailwind v4 defaults `border` to `currentColor` — and no gradients. Shared building blocks live in `src/components/`: `PageHeader.jsx` (top bar) and `Modal.jsx` (accessible dialog; there are no `window.prompt`/`confirm` calls — Home uses `Modal` for the new-provider and delete-provider flows).
  - **Toasts** use **sonner**. `<Toaster position="top-right" richColors closeButton />` is mounted once in `main.jsx`; components `import { toast } from 'sonner'` directly (no provider/hook). Prefer `toast.promise(request, { loading, success, error })` for async actions — it shows a loading spinner that transitions in place to success/error. Because `toast.promise`'s return value isn't the original promise, keep a reference to the request promise and chain navigation/refresh off *that* (`const request = api.x(); toast.promise(request, …); request.then(…)`).
